...
Future<List<Produto>> searchRecomend() async {
  final QuerySnapshot Marcas = await FirebaseFirestore.instance
      .collection('marcas')
      .get();

  List<Produto> listMarcas = [];
  for (QueryDocumentSnapshot produtoDoc in Marcas.docs) {
...
class _MarcasState extends State<Marcas> {
  ...
  void pesquisaMarcas() async {
    Marcas = await searchRecomend();
    print(Marcas.toString());
    setState(() {});
  }
  ...
  void search(String query) async {
    if (query.isEmpty) {
      ...
      List<Produto> MarcasEncontrados = await searchRecomend();
      items = MarcasEncontrados;
    }
  }
